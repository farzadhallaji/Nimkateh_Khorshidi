private void mediaDownload()
{
    try
    {
        ...
        {
            string response = client.DownloadString("http://localhost/advertising/api.php?msg=whatsupmedia");
            ...
            using (var conn = new SQLiteConnection(@"Data Source=" + Path.Combine(BaseDir, @"Database\mediadatabase.db")))
            {
                ...
                using (var cmd = new SQLiteCommand(conn))
                {
                    ...
                    {
                        cmd.CommandText = @"SELECT name,version from media";
                            ...
                            while (reader1.Read())
                            {
                                ...
                                foreach (JObject obj in json)
                                {
                                    ...
                                    if (name == obj["name"].ToString())
                                    {
                                        ...
                                        {
                                            File.Delete(@"Data\Media\" + obj["name"]);
                                            client.DownloadFileTaskAsync(new Uri("http://localhost/advertisng/media/" + obj["name"]+"?timestamp="+ DateTime.Now.ToString()), @"Data\Media\" + obj["name"]);
                ...
                if (newData.Count > 0)
                {
                    foreach (content obj in NewData)
                    {
                        using (var cmd = new SQLiteCommand(conn))
                        {
                            using (var transaction = conn.BeginTransaction())
                            {
                                cmd.CommandText = @"INSERT INTO media (name,version) VALUES(@name,@version)";
                                cmd.Parameters.AddWithValue("@name", obj.name);
                                cmd.Parameters.AddWithValue("@version", obj.version);
                                cmd.ExecuteNonQuery();
                                transaction.Commit();
                                client.DownloadFileTaskAsync(new Uri("http://localhost/advertising/media/" + obj.name + "?timestamp=" + DateTime.Now.ToString()), @"Data\Media\" + obj.name);
                            }
                        }
                    }
                    Setup();
                    ...
                    foreach (content obj in DeleteData)
                    {
                        using (var cmd = new SQLiteCommand(conn))
                        {
                            using (var transaction = conn.BeginTransaction())
                            {
                                ...
                                cmd.CommandText = @"UPDATE media set version=@version where id=@id";
